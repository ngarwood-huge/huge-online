 ...
 */
	...
	public static function getFilterHTML($filterArray) {
		...
		if ($show) {
			...
			$html .= '<li class="active '.$keyword.'-all"><a href="#" title="'.$keyword.'-all">'.jText::_('MOD_DIGI_SHOWCASE_FIELD_FILTER_ALL_LABEL').'</a></li>';
				...
				foreach($data As $filter) {
					...
					if ($filter || $filter == 0) {
						...
						if ($group && !in_array($filterTitle,$filtersArray) || !$group) {
							$html .= '<li class="'.$keyword.'-'.$filter.'"><a href="#" title="'.$keyword.'-'.$filter.'">'.$filterTitle.'</a></li>';
						}
