...
?>
	<div class="<?php echo ($i%2 ? 'odd' : 'even'); ?> panel panel-default" id="comment-item-<?php echo $id; ?>"><?php echo $comment; ?></div>
<?php
			} else {
?>
...
?>
<h4 class="product-section_title"><?php echo JText::_('COMMENTS_LIST_HEADER'); ?> <?php echo $btnRSS; ?><?php echo $btnRefresh; ?></h4>
<?php
	}
	...
	function getFooter()
	{
		...
		if ($this->getVar('comments-rss', 1) == 1) {
			$link = $this->getVar('rssurl');
			if (!empty($link)) {
				$lines[] = '<a class="rss" href="'.$link.'" title="'.JText::_('BUTTON_RSS').'" target="_blank"><i class="fa fa-rss-square"></i> '.JText::_('BUTTON_RSS').'</a>';
			}
		}
